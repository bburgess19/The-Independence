    ...
    useEffect(() => {
        const getArticles = async () => {
            try {
                ...
                setArticles(filteredData.sort((a, b) => new Date(a.upload_date) - new Date(b.upload_date)).slice(0, 6));
            } catch (err) {
                console.error(err);
            }
        };
            ...
            (entries) => {
                entries.forEach((entry) => {
                    if (entry.isIntersecting) {
                        ...
                        const nextThreeArticles = articles.filter((article) =>
                            !visibleArticles.includes(article)
                        ).slice(0, 3);
                        console.log("Visible:", visibleArticles);
                        console.log("Next:", nextThreeArticles);
                        console.log(nextThreeArticles);
                        setvisibleArticles((prevArticles) => [...prevArticles, ...nextThreeArticles]);
                        console.log("New Visible", visibleArticles);
                    }
                });
            },
            {threshold: 0.5}
